...
export default async function handler(req, res) {
  ...
  if (AUTO_FIX_WEBHOOK_SECRET) {
    ...
    if (auth !== `Bearer ${AUTO_FIX_WEBHOOK_SECRET}`) {
  ...
  if (payload.type !== 'INSERT') {
    return res.status(200).json({ skipped: true, reason: 'not INSERT' });
  }

  const report = payload.record;
  if (!report?.title || !report?.description) {
    return res.status(400).json({ error: 'record missing title or description' });
  }
  ...
  try {
    ...
    if (analysis.category === 'ui') {
      ...
      return res.status(200).json({ status: 'auto_merged', pr: pr.html_url });
    }
    ...
    return res.status(200).json({ status: 'pr_created', pr: pr.html_url });
